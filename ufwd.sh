...
# Forwards a UDP port from inside the namespace back to your normal host namespace
...
socat UNIX-LISTEN:./$sFile,reuseaddr,fork UDP4:0.0.0.0:$extPort &
children+=($!)
#echo "$!"

$portlCMD exec socat UDP6-LISTEN:$nsPort,reuseaddr,fork UNIX-CONNECT:./$sFile &
children+=($!)
#echo "$!"
