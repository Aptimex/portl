...
Requires GNU `sed`, `grep`, `cut`, and `tr` to be installed. If you get errors about unsupported flags in these commands it's probably because you have a BSD (POSIX) version installed. Port forwarding additionally requires `socat`, `setsid`, and `nsenter`.
...
- `fwd show`: List active forwards, including their protocol, ports, PID, and start time.
