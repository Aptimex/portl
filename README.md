...
### up
Uses the config file at `/etc/wireguard/portl0.conf` (created via the config command) to create a Wireguard interface `portl0` running in the `portl` network namespace.
...
### exec
Runs the specified command as the current user (even when `sudo` is used to run the script as root). To run the specified command as root just specify sudo as part of the target command. E.g. `sudo ./portl.sh exec sudo iptables -L`.
...
This script is the same as the original `portl.sh`, except it moves the wireguard interface into an existing docker container's namespace instead of creating a new namespace. As part of this process it also deletes all existing network interfaces inside the container, except for `lo` (loopback).
...
Usage is exactly the same as the original script described above, except the `up` command takes a single argument: the truncated 12-character `CONTAINER ID` of the target container, as displayed by `docker ps`. E.g. `dportl.sh up a1b2c3d4e5f6`
